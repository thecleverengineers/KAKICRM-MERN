...
export function CeoCommandPalette() {
  ...
  const query = useQuery({ queryKey: ['ceo-command-search', search], enabled: allowed && open && search.trim().length >= 2, queryFn: () => api<{ data: CommandResult[] }>(`/ceo/command-search${queryString({ q: search.trim() })}`) });
  useEffect(() => {
  ...
  return <div className="ceo-command-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}><section className="ceo-command-palette" role="dialog" aria-modal="true" aria-label="CEO command palette"><div className="ceo-command-search"><Search size={18} /><input id="ceo-command-input" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search employees, projects, tasks, invoices, clients…" /><kbd>ESC</kbd><button className="icon-button" type="button" onClick={close} aria-label="Close command palette"><X size={17} /></button></div>{search.trim().length < 2 ? <div className="ceo-command-section"><p className="eyebrow">QUICK COMMANDS</p>{quickCommands.map((command) => { const Icon = command.icon; return <button className="ceo-command-item" type="button" key={command.label} onClick={() => openTarget(command.href)}><span className="ceo-command-icon"><Icon size={16} /></span><span><strong>{command.label}</strong><small>{command.detail}</small></span><kbd>↵</kbd></button>; })}</div> : <div className="ceo-command-section"><p className="eyebrow">SEARCH RESULTS</p>{query.isPending && <p className="ceo-command-empty">Searching your authorised company records…</p>}{query.isError && <p className="ceo-command-empty">Search is temporarily unavailable.</p>}{!query.isPending && !query.isError && !results.length && <p className="ceo-command-empty">No matching employee, project, task, invoice, client or department.</p>}{results.map((result) => <button className="ceo-command-item" type="button" key={result.id} onClick={() => openTarget(result.href)}><span className="ceo-command-icon"><FileText size={16} /></span><span><strong>{result.label}</strong><small>{result.kind}{result.detail ? ` · ${result.detail}` : ''}</small></span><kbd>↵</kbd></button>)}</div>}<footer><span>CEO command palette</span><span><kbd>Ctrl</kbd> + <kbd>K</kbd> to open</span></footer></section></div>;
}
